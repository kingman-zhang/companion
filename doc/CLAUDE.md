...
/backend
  /routes          # API 路由
  /services        # scoring / LLM / 风控
  /models          # 数据模型
  /config          # 风控关键词、Prompt 配置
  /middleware      # 安全检查、鉴权
/ai
  chatAgent.ts           # 聊天 Agent
  rewriteAgent.ts        # 改写 Agent
  assessmentExplainer.ts # 评估解释 Agent
  safetyClassifier.ts    # 风控分类器
  emotionClassifier.ts   # 情绪分类器
```
